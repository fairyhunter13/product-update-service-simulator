...
func TestIntegration_MethodNotAllowed(t *testing.T) {
	...
	if resp1.StatusCode != http.StatusMethodNotAllowed { t.Fatalf("expected 405, got %d", resp1.StatusCode) }
	...
	if resp2.StatusCode != http.StatusMethodNotAllowed { t.Fatalf("expected 405, got %d", resp2.StatusCode) }
}
...
func TestIntegration_ContentTypeVariants(t *testing.T) {
	...
	for _, ctype := range variants {
		...
		if err != nil { t.Fatal(err) }
		resp.Body.Close()
		if resp.StatusCode != http.StatusAccepted { t.Fatalf("ctype %q expected 202, got %d", ctype, resp.StatusCode) }
	}
}
...
func TestIntegration_AckIncludesRequestIDAndTimestamp(t *testing.T) {
	...
	if a.RequestID != "itest-req-1" { t.Fatalf("request_id mismatch: %q", a.RequestID) }
	if _, err := time.Parse(time.RFC3339, a.ReceivedAt); err != nil { t.Fatalf("received_at not RFC3339: %v", err) }
}
...
func TestIntegration_LastWriteWins(t *testing.T) {
	...
	var p struct{ Price float64 `json:"price"`; ProductID string `json:"product_id"` }
	if err := json.NewDecoder(resp3.Body).Decode(&p); err != nil { t.Fatal(err) }
	if p.ProductID != "lww" || p.Price != 2 { t.Fatalf("unexpected product: %+v", p) }
}
