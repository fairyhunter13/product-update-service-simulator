...
func TestIntegration_MetricsIncreaseAndSane(t *testing.T) {
	...
	if resp0.StatusCode != http.StatusOK { t.Fatalf("expected 200, got %d", resp0.StatusCode) }
	if err := json.NewDecoder(resp0.Body).Decode(&before); err != nil { t.Fatal(err) }
	...
	if err := json.NewDecoder(resp1.Body).Decode(&after); err != nil { t.Fatal(err) }

	bProc := toFloat(before["events_processed"]) 
	aProc := toFloat(after["events_processed"]) 
	if aProc < bProc { t.Fatalf("events_processed did not increase: before=%v after=%v", bProc, aProc) }

	uptime := toFloat(after["uptime_sec"]) 
	if uptime < 0 { t.Fatalf("uptime_sec negative: %v", uptime) }

	w := toFloat(after["worker_count"]) 
	if w <= 0 { t.Fatalf("worker_count should be > 0, got %v", w) }
}
...
func TestIntegration_ResponseContentTypeHeaders(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	defer resp1.Body.Close()
	if ct := resp1.Header.Get("Content-Type"); ct == "" || ct[:16] != "application/json" { t.Fatalf("unexpected content-type: %q", ct) }
	// healthz content-type
	resp2, err := http.Get(u+"/healthz")
	if err != nil { t.Fatal(err) }
	defer resp2.Body.Close()
	if ct := resp2.Header.Get("Content-Type"); ct == "" || ct[:16] != "application/json" { t.Fatalf("unexpected content-type: %q", ct) }
}
...
func TestIntegration_GeneratedRequestIDWhenMissing(t *testing.T) {
	...
	if resp.StatusCode != http.StatusAccepted { t.Fatalf("expected 202, got %d", resp.StatusCode) }
	var a ack2
	if err := json.NewDecoder(resp.Body).Decode(&a); err != nil { t.Fatal(err) }
	if a.RequestID == "" { t.Fatalf("expected generated request_id when header missing") }
}
